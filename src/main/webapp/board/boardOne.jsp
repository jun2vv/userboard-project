...
<body>
	<div class="container">
		...
		if (session.getAttribute("loginMemberId") != null
				&& session.getAttribute("loginMemberId").equals(boardRs.getString("memberId"))) {
		%>
		<div>
		...
		<form
			...
			<table class="table table-striped">
				<tr>
					...
					<td><textarea rows="2" cols="60" name="commentContent"></textarea>
					</td>
				</tr>
			</table>
			<button class="btn btn-outline-dark" type="submit">댓글달기</button>
		</form>
		<br>
		<%
		} else {
		%>
			<div>로그인시에만 댓글 입력이 가능합니다.</div>
		<%
		} 
		%>
		...
		<table class="table table-striped">
			...
			<tr>
				...
				<td><a
					href="<%=request.getContextPath()%>/board/modifyCommentForm.jsp?commentNo=<%=c.getCommentNo()%>&boardNo=<%=c.getBoardNo()%>">수정</a>
				</td>
				<td><a
					href="<%=request.getContextPath()%>/board/removeCommentAction.jsp?commentNo=<%=c.getCommentNo()%>&boardNo=<%=c.getBoardNo()%>">삭제</a>
				</td>
				<%
				}
				%>
		...
		<div>
			...
			%>
			<a
				href="<%=request.getContextPath()%>/board/boardOne.jsp?boardNo=<%=boardNo%>&currentPage=<%=currentPage - 1%>">이전</a>
			<%
			}
			if (currentPage < lastPage) {
			%>
			<a
				href="<%=request.getContextPath()%>/board/boardOne.jsp?boardNo=<%=boardNo%>&currentPage=<%=currentPage + 1%>">다음</a>
			<%
			}
			%>
